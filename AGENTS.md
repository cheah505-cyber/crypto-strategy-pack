...
- 修改前必须用 `python tools/handoff.py start --owner <agent> --session <唯一ID> --file <文件> --paths <路径>` 取得限时占用；不得覆盖未过期的他人租约。暂停、阻断、完成分别用 `pause`、`block`、`complete`；恢复 GitHub Actions 责任使用 `monitor` 并显式填写载体、目标、间隔和过期阈值，再提交推送并严格运行 `python tools/check_handoff.py`。
- 严格检查默认刷新远端；失败必须报 `UNVERIFIED`。`--offline` 只验证本地结构，`--allow-dirty` 只供提交前检查。
- `MONITORING` 只允许 `.handoff-monitoring.json` 白名单内的 GitHub Actions 提交使远端领先，并自动验证远端健康与 paper state 一致性；本地领先、分叉、脏工作区或非白名单提交均不能交付。
